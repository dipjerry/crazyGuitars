...
if (isset($_GET['delgroup'])) {
	...
	$stmt = $db->prepare('DELETE FROM crazyguitar_group WHERE groupID = :groupID');
	$stmt->execute(array(':groupID' => $_GET['delgroup']));

	header('Location: album.php?action=deleted');
	exit;
}
...
<body>
	...
	<div class="w3-content  w3-center w3-padding-64" style="max-width:max-content">
		<div id="w3-display-container w3-center w3-padding-64 ">
			...
			<table class="w3-table w3-bordered w3-hoverable w3-card" style="width:100%">
				...
				try {
					...
					$stmt = $db->query('SELECT groupID, groupTitle, groupSlug FROM crazyguitar_group ORDER BY groupTitle DESC');
					while ($row = $stmt->fetch()) {

						echo '<tr>';
						echo '<td class="w3-center w3-padding-small">' . $row['groupTitle'] . '</td>';
				?>

						<td class="w3-center w3-padding-small">
							<a href="edit-album-<?php echo $row['groupID']; ?>">Edit</a> |
			...
			<p><a href='add-album.php' class="w3-btn w3-red">Add Album</a></p>
	...
	<script language="JavaScript" type="text/javascript">
		function delgroup(id, title) {
			if (confirm("Are you sure you want to delete '" + title + "'")) {
				window.location.href = 'album.php?delgroup=' + id;
			}
		}
	</script>
</body>
