...
if (isset($_GET['deluser'])) {
	...
	if ($_GET['deluser'] != '1') {
		...
		$stmt = $db->prepare('DELETE FROM crazyguitar_artist WHERE memberID = :memberID');
		...
		<div id="w3-display-container w3-center w3-padding-64">
			...
			<table class="w3-table w3-bordered w3-hoverable w3-card ">
				...
				try {
					...
					$stmt = $db->query('SELECT memberID, name, email , designation , disply_pic  FROM crazyguitar_artist ORDER BY name');
				...
				?>
						<td>
							<a href="edit-artist-<?php echo $row['memberID']; ?>">Edit</a>
			...
			<p><a class="w3-btn w3-red" href='add-artist'>Add Artist</a></p>
		</div>
	</div>
</body>
