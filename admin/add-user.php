...
<body>
	...
	<div id="wrapper">
		...
		if (isset($_POST['submit'])) {
			...
			$stmt3 = $db->prepare('SELECT username, email FROM blog_members where username=:username or email=:email');
			$stmt3->execute(array(
				':username' => $username,
				':email' => $email
			));
			...
			if (!isset($error)) {
				...
				try {
					...
					$stmt = $db->prepare('INSERT INTO blog_members (username,password,email,about, display_pic) VALUES (:username, :password, :email,:about, :display_pic)');
					$stmt->execute(array(
						':username' => $username,
