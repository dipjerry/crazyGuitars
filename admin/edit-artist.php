...
<body>
	<div class="w3-content  w3-padding-64" style="max-width:max-content">
		<div id="w3-display-container w3-padding-64">
			...
			<p><a href="artist.php">Artist Admin Index</a></p>
			<h2>Edit User</h2>
			<?php
			...
			if (isset($_POST['submit'])) {
				...
				if (!isset($error)) {
					//update database
					$stmt = $db->prepare('UPDATE crazyguitar_artist SET name = :name, email = :email, about = :about, disply_pic = :disply_pic WHERE memberID = :memberID');
					$stmt->execute(array(
						':name' => $name,
						':email' => $email,
						':about' => $about,
				...
				header('Location: artist.php?action=updated');
				exit;
			}
			?>
			<?php
