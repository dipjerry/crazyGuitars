...
$stmt = $db->prepare('SELECT postID, postTitle,youtube_link,postDesc,postThumb, postCont, postKey,  postDate, postSlug, author, views FROM crazyguitar_posts_seo WHERE postSlug = :postSlug');
...
<body itemscope itemtype="http://schema.org/Article">
	...
	$stmtC = $db->prepare('UPDATE crazyguitar_posts_seo SET  views = :counter WHERE postSlug = :postSlug');
	$stmtC->execute(array(
		':counter' => $counter,
		':postSlug' =>  $_GET['id']
	));
	...
	<div class="w3-row" style="margin-top:3%;">
		...
		<div class="w3-col l8 s12">
			<div class="w3-card-4 w3-margin w3-white">
				<p itemscope itemtype="https://schema.org/BreadcrumbList" class="breadCrumb-p"><?= breadcrumbs() ?>
					<meta itemprop="name" content="<?php echo $row['postTitle']; ?>" />
				</p>
				<?php if ($row['postThumb'] != '') { ?>
					<img itemprop="image" src="./uploads/<?php echo $row['postThumb'] ?>" alt="<?php echo $row['postThumb'] ?>" style="width:100%">
				<?php } ?>
				<div class="w3-container">
					...
					?>
						<a href="<?php echo $row['youtube_link'] ?>" target="_blank">
							<div class="w3-container w3-metro-red w3-card">
								<h2>View on Youtube</h2>
			...
			<div class="w3-card-4 w3-margin w3-red">
				<div class="w3-container w3-center share_buttons">
					...
					<ul>
						...
						<span>
							<a target="_blank" href="http://www.facebook.com/sharer.php?u=http://<?php echo $baseUrl . $slug; ?>">
								<i class="fa fa-facebook-f fa_custom-f hvr-wobble-vertical" style="font-size:24px"></i>
							</a>
						</span>
						<span>
							<a target="_blank" href="http://twitter.com/share?text=Visit the link &url=http://<?php echo $baseUrl . $slug; ?>&hashtags=<?php echo $hash  ?> ">
								<i class="fa fa-twitter fa_custom-t hvr-wobble-vertical " style="font-size:24px"></i>
							</a>
						</span>
						<span>
							<a target="_blank" href="http://www.linkedin.com/shareArticle?mini=true&url=http://<?php echo $baseUrl . $slug; ?>">
